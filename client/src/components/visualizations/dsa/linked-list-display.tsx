...
function ListNode({
  ...
  return (
    <motion.div
      ...
      className="relative flex-shrink-0 w-16 h-16 rounded-lg flex items-center justify-center border border-gray-200"
    >
      {showPrevArrow && (
        <motion.div
        ...
        >
          <div className="relative w-full">
            ...
            <ArrowLeft className="h-4 w-4 text-gray-400 absolute left-0 -translate-x-[2px] top-1/2 -translate-y-1/2" />
          </div>
        </motion.div>
      )}
      <span className={`text-lg font-mono ${isHighlighted ? "text-white" : "text-gray-700"}`}>
        {displayValue}
      </span>
    </motion.div>
  );
  ...
  if (isCurved) {
    return (
      <motion.div
        ...
        exit={{ opacity: 0 }}
      >
        <div className={`h-16 w-[2px] ${isHighlighted ? "bg-brand-primary" : "bg-gray-300"}`} />
        <div className="relative w-32 h-8">
          <div
            className={`absolute inset-0 border-t-2 border-r-2 rounded-tr-2xl ${
              isHighlighted ? "border-brand-primary" : "border-gray-300"
            }`}
          />
          <ArrowRight
            className={`h-4 w-4 absolute right-0 translate-x-[2px] top-1/2 -translate-y-1/2 ${
              isHighlighted ? "text-brand-primary" : "text-gray-400"
            }`}
          />
        </div>
      </motion.div>
  ...
  return (
    ...
    >
      <div className="relative w-full -translate-y-2">
        <div className={`h-[2px] w-full ${isHighlighted ? "bg-brand-primary" : "bg-gray-300"}`} />
        <ArrowRight
          className={`h-4 w-4 absolute right-0 translate-x-[2px] top-1/2 -translate-y-1/2 ${
            isHighlighted ? "text-brand-primary" : "text-gray-400"
          }`}
...
export function LinkedListDisplay({
  ...
  return (
    <Card className="p-6 relative bg-white border border-gray-200">
      {message && (
        <div className="absolute top-4 left-4 text-sm text-gray-500">{message}</div>
      )}
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2">
          <div className="flex flex-col items-center">
            <NextArrow
              ...
              isCurved
            />
            <span className="text-xs text-gray-500 mt-2">Circular Connection</span>
          </div>
        </div>
      )}
    </Card>
